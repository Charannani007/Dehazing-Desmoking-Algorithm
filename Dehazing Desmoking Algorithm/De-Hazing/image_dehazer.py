...
class image_dehazer():
    ...
    def remove_haze(self, img, airlightEstimation_windowSze=15, boundaryConstraint_windowSze=3, C0=20, C1=300,
        ...
        self.__AirlightEstimation(img)
        self.__BoundCon(img)
        self.__CalTransmission(img)
        HazeCorrectedImg = self.__removeHaze(img)
        return HazeCorrectedImg, self._Transmission
    ...
    def __psf2otf(self, psf, outSize):
        ...
        otf = np.fft.fft2(psf)
        n_ops = np.sum(psfSize * np.log2(psfSize))
